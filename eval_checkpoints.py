...
flags.DEFINE_bool("do_predict", False, "Whether to run prediction on the dev set.")
...
flags.DEFINE_bool("use_next_sentence_loss", True, "Whether to use next sentence loss.")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if FLAGS.use_next_sentence_loss:
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      predictions['masked_lm_log_probs'] = masked_lm_log_probs[tf.newaxis]
      predictions['masked_lm_predictions'] = tf.argmax(masked_lm_log_probs[tf.newaxis],
...
def input_fn_builder(input_files,
  ...
  def input_fn(params):
    ...
    if not is_training and not is_evaluating:
        d = d.repeat(1)
        #d = d.shard(num_shards = 100, index = 0)
        #d = d.take(1)
        pass
...
def main(eval_time):
  ...
  if FLAGS.do_predict:
    ...
    if FLAGS.eval_file:
        output_eval_file = FLAGS.eval_file
    else:
        output_eval_file = os.path.join(FLAGS.output_dir, "eval_results.pkl")

    if not os.path.exists(os.path.dirname(output_eval_file)):
        os.makedirs(os.path.dirname(output_eval_file))

    f = open(output_eval_file, 'wb')
    pickle.dump(results, f)
    f.close()
    print('%d predictions written to `%s`' % (len(results), output_eval_file))
